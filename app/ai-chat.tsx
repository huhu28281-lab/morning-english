...
export default function AiChat({conversation,onConversation,level,rate,quiet,connections,onConnect,onPractice}:{conversation:Conversation;onConversation:(v:Conversation)=>void;level:"work"|"basics";rate:number;quiet:boolean;connections:ReturnType<typeof useConnections>;onConnect:()=>void;onPractice:(text:string)=>void}) {
  ...
  return <div className="feature-page">
    <div className="page-heading"><div><p className="eyebrow">A CONVERSATION, ONE TURN AT A TIME</p><h1>이제, 대화를 이어가요.</h1><p className="heading-sub">내 이야기에 답하고 다시 물어보는 AI 영어 파트너.</p></div><button className="secondary-button" onClick={onConnect}><Settings2 size={17}/>AI 연결</button></div>
    <ConnectionNotice provider="cloudflare" connections={connections} onOpen={onConnect}/>
    {connections.status?.usage && <div className="cf-usage-bar" role="status"><div><strong>오늘 {connections.status.usage.calls} / {connections.status.usage.maxCalls}회 요청</strong><span>앱 사용량 예산 {connections.status.usage.reservedNeurons.toLocaleString()} / {connections.status.usage.maxNeurons.toLocaleString()} · 한국 시간 오전 9시 초기화</span></div><p>보수적으로 예약한 앱 사용량이며 Cloudflare의 실제 잔여량은 아니에요. 실패한 요청도 포함합니다.</p></div>}
    <div className="chat-layout"><section className="feature-panel chat-panel"><div className="chat-toolbar"><div><span className="partner-avatar">m<span>.</span></span><div><strong>Morning</strong><small>{connections.status?.cloudflare?"Cloudflare · 영어 회화 파트너":"연결 후 대화를 시작할 수 있어요"}</small></div></div><label htmlFor="chat-translation">해석 보기<Switch id="chat-translation" checked={translated} onCheckedChange={setTranslated}/></label></div>
