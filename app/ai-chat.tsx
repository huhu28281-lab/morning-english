...
export default function AiChat({conversation,onConversation,level,rate,quiet,connections,onConnect,onPractice}:{conversation:Conversation;onConversation:(v:Conversation)=>void;level:"work"|"basics";rate:number;quiet:boolean;connections:ReturnType<typeof useConnections>;onConnect:()=>void;onPractice:(text:string)=>void}) {
  ...
  return <div className="feature-page">
    ...
    {connections.status?.usage && <div className="cf-usage-bar" role="status"><div><strong>오늘 {connections.status.managedCloudflare?"앱 전체 ":""}{connections.status.usage.calls} / {connections.status.usage.maxCalls}회 요청</strong><span>앱 사용량 예산 {connections.status.usage.reservedNeurons.toLocaleString()} / {connections.status.usage.maxNeurons.toLocaleString()} · 한국 시간 오전 9시 초기화</span></div><p>{connections.status.managedCloudflare?"모든 방문자가 함께 사용하는 한도예요. ":""}횟수 또는 사용량 예산 중 먼저 도달한 한도에서 멈추며 실패한 요청도 포함합니다.</p></div>}
