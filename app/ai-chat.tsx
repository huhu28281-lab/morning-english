...
import { Send, Mic, Volume2, RotateCcw, MessageCircle, Loader2, ArrowRight } from "lucide-react";
import { Textarea } from "@/components/ui/textarea";
import { Switch } from "@/components/ui/switch";
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select";
import { useSpeech } from "./use-speech";
import { ConnectionNotice, type useConnections } from "./use-connections";
...
export default function AiChat({conversation,onConversation,level,rate,connections,onPractice}:{conversation:Conversation;onConversation:(v:Conversation)=>void;level:"work"|"basics";rate:number;connections:ReturnType<typeof useConnections>;onPractice:(text:string)=>void}) {
  ...
  return <div className="feature-page">
    <div className="page-heading"><div><p className="eyebrow">A CONVERSATION, ONE TURN AT A TIME</p><h1>AI와 영어 대화</h1><p className="heading-sub">상황을 고르고 영어로 이야기를 나눠보세요.</p></div></div>
    <ConnectionNotice provider="cloudflare" connections={connections}/>
    <div className="chat-layout"><section className="feature-panel chat-panel"><div className="chat-toolbar"><div><span className="partner-avatar">m<span>.</span></span><div><strong>Morning</strong><small>{connections.status?.cloudflare?"영어 회화 파트너":connections.status?.managedCloudflare?"현재 대화를 이용할 수 없어요":"이용 가능 여부 확인 중"}</small></div></div><label htmlFor="chat-translation">해석 보기<Switch id="chat-translation" checked={translated} onCheckedChange={setTranslated}/></label></div>
