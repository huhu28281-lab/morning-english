...
export default function AiSettings({open,onOpenChange,connections}:{open:boolean;onOpenChange:(v:boolean)=>void;connections:ReturnType<typeof useConnections>}) {
  ...
  return <Dialog open={open} onOpenChange={v=>{if(!busy)onOpenChange(v);}}><DialogContent className="feature-dialog ai-settings-dialog"><DialogHeader><DialogTitle>Cloudflare로 영어 대화 연결</DialogTitle><DialogDescription>무료 사용량을 활용하는 설정이에요. 토큰은 서버에서 암호화해 보관합니다.</DialogDescription></DialogHeader>
    ...
    {connections.status && !connections.status.ready && <p className="form-error">연결 설정을 준비 중이에요. 잠시 후 다시 열어주세요.</p>}
    <section className="provider-settings"><div className="feature-panel-top"><h3>영어 대화 · Cloudflare Workers AI</h3>{connections.status?.cloudflare && <span className="connection-saved"><Check size={14}/>설정됨</span>}</div>
