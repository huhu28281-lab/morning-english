  ...
  return <div className="app-shell">
    ...
    <Tabs value={tab} onValueChange={changeTab} orientation={wideLayout ? "vertical" : "horizontal"} className={`workspace-tabs tab-${tab}`}>
      ...
      <TabsContent value="today" id="study-main">{weeklyBanner}
        <div className="page-heading today-heading"><div><p className="eyebrow">MAKE TIME FOR YOURSELF</p><h1>오늘의 회화</h1><p className="heading-sub">{level === "work" ? "상황을 듣고, 내 말로 답해보세요." : "짧은 문장부터 차근차근 연습해보세요."}</p></div><div className="today-progress"><span className="progress-kicker">DAY {number(dayNumber(day))}</span><strong>{loading || loadError ? "—" : done.length}<span> / 6</span></strong><span>이번 수업 완료 단계</span><Progress value={done.length/6*100} aria-label="이번 수업 완료 단계"/></div></div>
